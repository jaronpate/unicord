...
export class UnicordDiscordAPI {
    private readonly base_url: string = 'https://discord.com/api';
    private readonly version: number = 10;

    constructor(private self: Unicord) {}

    async request<T = any>(method: string, path: string, data?: any): Promise<T> {
        const url = `${this.base_url}/v${this.version.toString()}${path.startsWith('/') ? '' : '/'}${path}`;
        const headers = {
            Authorization: `Bot ${this.self.config.token}`,
