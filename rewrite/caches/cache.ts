...
import { Trait } from '../types/common';

export class UnicordObjectCache<Entity> {
    cache: LRUCache<string, Entity>;
    requests = new Map<string, Promise<Entity>>();
    readonly type: string;

    constructor(
        private readonly self: Unicord,
        type: string,
        private readonly factory: { [Trait.fromDiscord]: (data: any) => Entity } & (new (data: any) => Entity),
        events: string[],
    ...
    set(object_id: string, object: Entity): void {
    ...
    async fetch(object_id: string, ..._args: any[]): Promise<Entity> {
        if (this.requests.has(object_id)) {
            ...
            const req = this.self.discordAPI.get(`/${this.type}/${object_id}`).then((data) => {
                // Create the object
                const object = this.factory[Trait.fromDiscord](data);
                // Save the object
                this.cache.set(object_id, object);
                // Return the object
                return object;
    ...
    async get(object_id: string, ..._args: any[]): Promise<Entity> {
